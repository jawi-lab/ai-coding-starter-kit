      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!availLoading && !availError && totalMembers > 0 && membersWithoutCalendar > 0 && (
            <div className="mx-5 mt-4 flex items-start gap-2.5 bg-secondary-soft border border-secondary/25 rounded-md px-3.5 py-2.5">
          ...
          {!availLoading && !availError && cachedAt && (
            <div className="mx-5 mt-3 flex items-center justify-between gap-2 text-[12px] text-ink-3">
          ...
          {/* Error state */}
          {availError && (
            <div className="mx-5 mt-6 flex flex-col items-center gap-3 bg-surface border border-line rounded-md p-5 text-center">
          ...
          {/* Legend */}
          {!availLoading && (
            <div className="mx-5 mt-3.5 mb-1 flex flex-wrap items-center gap-x-4 gap-y-1.5">
          ...
          {/* Calendar — full-width months, one below the other */}
          {!availLoading && (
            <div className="px-5 pb-6">
              <Calendar
                ...
                className="mx-auto w-full max-w-[440px] bg-transparent p-0"
                classNames={{
                  root: 'w-full',
                  months: 'flex flex-col gap-8',
                  month: 'flex w-full flex-col gap-3',
                  month_caption: 'flex h-auto w-full items-center px-1 pt-4',
                  caption_label: 'select-none text-[16px] font-[700] text-ink',
                  weekdays: 'flex px-0',
                  weekday: 'flex-1 select-none text-[11px] font-[600] text-ink-3',
                  week: 'mt-1.5 flex w-full',
                  // Round the ends of each contiguous availability band (week edges)
                  day:
                    'group/day relative aspect-square h-full w-full select-none p-0 text-center ' +
                    '[&:first-child]:rounded-l-[10px] [&:last-child]:rounded-r-[10px] ' +
                    '[&:first-child[data-selected=true]_button]:rounded-l-[10px] ' +
                    '[&:last-child[data-selected=true]_button]:rounded-r-[10px]',
                  range_start: 'rounded-l-[10px] bg-transparent',
                  range_middle: 'rounded-none',
                  range_end: 'rounded-r-[10px] bg-transparent',
                  today: '[&_button]:font-[800]',
                }}
                components={{
                  Nav: () => <></>,
                }}
